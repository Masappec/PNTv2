...
import EstablishmentListPresenter from './EntidadesCumplimientoPresenter'

const EntidadesyCumplimientoContainer = ({ usecase }: { usecase?: EstablishmentUseCase }) => {
  ...
  const handleConfirmDelete = () => {
    if (selectedEstablishment) {
      usecase
        ?.delete(selectedEstablishment.id + '')
        .then(() => {
          setVisibleModal(false)
          setSelectedEstablishment(null)
          usecase
  ...
  const onChangeFilter = (name: string) => {
    ...
    if (name === '') {
      ...
      usecase
        ?.getEstablishments('', currentPage, name)
        .then(response => {
          setEstablishments(response.results)
          setCurrentPage(response.current)
          setFrom(response.from || 1)
...
export default EntidadesyCumplimientoContainer
