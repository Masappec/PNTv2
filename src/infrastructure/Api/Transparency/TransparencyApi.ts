...
class TransparencyApi {
  ...
  async getTransparencyActive(id_establishment?: number, page: number = 1) {
    try {
      const response = await this._api.get<Pagination<PublicationResponse>>(
        `${TRANSPARENCY_PATH}/public/transparency/active/list`,
        {
          params: {
  ...
  async getDetailTransparency(id: number) {
    try {
      const response = await this._api.get<PublicationResponse>(
        `${TRANSPARENCY_PATH}/public/transparency/active/${id}`
      );
  ...
  async getListTransparency() {
    try {
      const response = await this._api.get <Pagination<PublicationResponse>>(
        `${TRANSPARENCY_PATH}/publications/list`
      );
      return response.data;
    } catch (error) {
      if (error instanceof AxiosError) {
