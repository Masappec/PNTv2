...
class NumeralService {
  ...
  async getNumeralByEstablishment(
    establishment_id: number,
    year?: number,
    month?: number,
  ) {
    const res = await this.api.getNumeralByEstablishment(
      establishment_id,
      year,
      month,
    );
  ...
  /**
   * Actualiza el estado de un numeral (isDefault)
  ...
  async updateNumeralState(numeral_id: number, data: { isDefault: boolean }) {
    ...
    //return NumeralDetailMapper.toDomain(res); // Es necesario utilizar NumeralDetail Mapper
    return res;
  }
}
