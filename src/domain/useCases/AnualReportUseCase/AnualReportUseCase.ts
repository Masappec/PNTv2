  ...
  async createAnualReport(data: AnualReportEntity) {
    ...
    if (data.have_public_records){
      if(data.norme_archive_utility.trim().length === 0){
        throw new Error("Norma de utilidad de archivo no puede estar vacía");
      }
    ...
    if (data.have_quality_problems){
      ...
      if (data.description_quality_problems.trim().length === 0){
        throw new Error("Descripción específica de la corrección de información no puede estar vacía");
      }
    }

    if(data.have_sanctions){
      if (data.total_organic_law_public_service === 0){
        throw new Error("Cantidad de sanciones por la ley orgánica del servicio público no puede ser 0");
      }

      if (data.description_organic_law_public_service.trim().length === 0){
        throw new Error("Descripción específica de sanciones por la ley orgánica del servicio público no puede estar vacía");
      }

      if (data.total_organic_law_contraloria === 0){
        throw new Error("Cantidad de sanciones por la ley orgánica de la contraloría no puede ser 0");
      }

      if (data.description_organic_law_contraloria.trim().length === 0){
        throw new Error("Descripción específica de sanciones por la ley orgánica de la contraloría no puede estar vacía");
      }

      if (data.total_organic_law_national_system === 0){
        throw new Error("Cantidad de sanciones por la ley orgánica del sistema nacional de control no puede ser 0");
      }

      if (data.description_organic_law_national_system.trim().length === 0){
        throw new Error("Descripción específica de sanciones por la ley orgánica del sistema nacional de control no puede estar vacía");
      }

      if (data.total_organic_law_citizen_participation === 0){
        throw new Error("Cantidad de sanciones por la ley orgánica de participación ciudadana no puede ser 0");
      }

      if (data.description_organic_law_citizen_participation.trim().length === 0){
        throw new Error("Descripción específica de sanciones por la ley orgánica de participación ciudadana no puede estar vacía");
      }
    ...
    if (data.did_you_entity_receive){
      if (data.total_saip_in_portal === 0){
        throw new Error("Cantidad de solicitudes de información pública recibidas en el portal no puede ser 0");
      }

      if (data.total_saip_no_portal === 0){
        throw new Error("Cantidad de solicitudes de información pública recibidas fuera del portal no puede ser 0");
      }

      if (data.description_rason_no_portal.trim().length === 0){
        throw new Error("Descripción de razón de solicitudes no recibidas en el portal no puede estar vacía");
      }

      if (data.have_responded_solicities_no_portal){
        if (data.total_no_registered === 0){
          throw new Error("Cantidad de solicitudes no registradas no puede ser 0");
        }

        if (data.comment_aclaration_no_registered.trim().length === 0){
          throw new Error("Comentario aclaratorio de solicitudes no registradas no puede estar vacío");
        }
      }
